 ...
 *
 * <p>
 * The mapper handles the small amount of request-body validation that is awkward to
 * express directly in controller signatures: path/body ID consistency, tenant
 * consistency, and the optional nested authentication payload.
 */
	...
	TaskPushNotificationConfig parseCreateRequest(String body, String taskId, @Nullable String tenant) {
		...
		if (!bodyTenant.isBlank() && tenant != null && !tenant.isBlank() && !bodyTenant.equals(tenant)) {
			throw new InvalidParamsError(
					"Tenant in request body (" + bodyTenant + ") does not match tenant in URL path (" + tenant + ").");
		}
