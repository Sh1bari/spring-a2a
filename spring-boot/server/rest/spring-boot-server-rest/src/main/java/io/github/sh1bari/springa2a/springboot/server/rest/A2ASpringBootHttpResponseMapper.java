 ...
 *
 * <p>
 * The mapper keeps protocol serialization in one place so the controller can stay thin
 * and focused on request extraction and delegation. It also owns the SSE conversion for
...
public final class A2ASpringBootHttpResponseMapper {
	...
	ResponseEntity<String> ok(Object body) {
		return ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body(toJson(body));
	}
	...
	ResponseEntity<String> okSerialized(String body, Map<String, String> headers) {
		ResponseEntity.BodyBuilder builder = ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON);
		headers.forEach(builder::header);
	...
	ResponseEntity<String> created(Object body) {
		return ResponseEntity.status(201).contentType(MediaType.APPLICATION_JSON).body(toJson(body));
	}
	...
	ResponseEntity<Void> notModified(Map<String, String> headers) {
		ResponseEntity.BodyBuilder builder = ResponseEntity.status(HttpStatus.NOT_MODIFIED);
		headers.forEach(builder::header);
	...
	SseEmitter toSseEmitter(Flow.Publisher<StreamingEventKind> publisher, ServerCallContext context) {
		...
		java.util.concurrent.atomic.AtomicReference<java.util.concurrent.Flow.Subscription> subscriptionRef = new java.util.concurrent.atomic.AtomicReference<>();
	...
	private ResponseEntity<String> okProto(com.google.protobuf.MessageOrBuilder proto) {
		return ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body(toProtoJson(DEFAULT_PRINTER, proto));
	}
	...
	private record ErrorBody(int code, String status, String message, List<ErrorDetail> details) {
		private ErrorBody(A2AError error) {
			this(mapErrorToHttpStatus(error), A2AErrorCodes.fromCode(error.getCode()) != null
					? A2AErrorCodes.fromCode(error.getCode()).grpcStatus() : A2AErrorCodes.INTERNAL.grpcStatus(),
					...
					List.of(ErrorDetail.of(
							A2AErrorCodes.fromCode(error.getCode()) != null
									? A2AErrorCodes.fromCode(error.getCode()).name() : A2AErrorCodes.INTERNAL.name(),
							error.getDetails())));
		}
	}
