...
class A2ASpringBootServerContractTest extends AbstractA2AServerTest {
	...
	static class TestApplication {
		...
		@Bean
		RequestHandler requestHandler(AgentExecutor agentExecutor, TaskStore taskStore, QueueManager queueManager,
				PushNotificationConfigStore pushNotificationConfigStore, MainEventBusProcessor mainEventBusProcessor,
			...
			return new DefaultRequestHandler(agentExecutor, taskStore, queueManager, pushNotificationConfigStore,
					mainEventBusProcessor, internalExecutor, eventConsumerExecutor);
		}
		...
		private AgentCard testAgentCard() {
			return AgentCard.builder()
				...
				.skills(List.of())
				.supportedInterfaces(
						List.of(new AgentInterface(HTTP_JSON.asString(), "http://localhost:" + SERVER_PORT)))
				.build();
		}
	...
	static class TestSupportController {
		...
		TestSupportController(TaskStore taskStore, QueueManager queueManager,
				PushNotificationConfigStore pushNotificationConfigStore, AtomicInteger streamingSubscribedCount) {
		...
		@PostMapping(value = "/test/queue/enqueueTaskStatusUpdateEvent/{taskId}",
		...
		@PostMapping(value = "/test/queue/enqueueTaskArtifactUpdateEvent/{taskId}",
				consumes = MediaType.APPLICATION_JSON_VALUE)
		...
		ResponseEntity<Void> savePushNotificationConfig(@PathVariable String taskId, @RequestBody String body)
				throws Exception {
			TaskPushNotificationConfig notificationConfig = JsonUtil.fromJson(body, TaskPushNotificationConfig.class);
			pushNotificationConfigStore
				.setInfo(TaskPushNotificationConfig.builder(notificationConfig).taskId(taskId).build());
			return ResponseEntity.ok().build();
		}

		@PostMapping("/test/queue/awaitChildCountStable/{taskId}/{expectedCount}/{timeoutMs}")
		ResponseEntity<String> awaitChildCountStable(@PathVariable String taskId, @PathVariable String expectedCount,
	...
	static final class TestAgentExecutor implements AgentExecutor {
		...
		public void execute(org.a2aproject.sdk.server.agentexecution.RequestContext context,
			...
			if (input.startsWith("multi-event:second")) {
				agentEmitter.addArtifact(List.of(new TextPart("Second message artifact")), "artifact-2",
						"Second Artifact", null);
				agentEmitter.complete();
				return;
			}
			if (input.startsWith("input-required:")) {
				...
				agentEmitter.requiresInput(
						agentEmitter.newAgentMessage(List.of(new TextPart("Please provide additional information")),
								context.getMessage().metadata()));
				return;
			}
			if (input.startsWith("auth-required:")) {
				agentEmitter.requiresAuth(
						agentEmitter.newAgentMessage(List.of(new TextPart("Please authenticate with OAuth provider")),
								context.getMessage().metadata()));
				try {
					Thread.sleep(2000);
				}
		...
		private void handleDelegation(String userInput, TransportProtocol transportProtocol,
			...
			try (org.a2aproject.sdk.client.Client client = AgentToAgentClientFactory.createClient(agentCard,
					transportProtocol)) {
				agentEmitter.startWork();
				java.util.concurrent.atomic.AtomicReference<Task> taskRef = new java.util.concurrent.atomic.AtomicReference<>();
				...
				if (!delegatedResult.status().state().isFinal()) {
					String diagnostic = String.format(
							...
							delegatedResult.status().state(), delegatedResult.id(),
		...
		private void handleLocally(String userInput, org.a2aproject.sdk.server.tasks.AgentEmitter agentEmitter) {
			try {
				agentEmitter.startWork();
