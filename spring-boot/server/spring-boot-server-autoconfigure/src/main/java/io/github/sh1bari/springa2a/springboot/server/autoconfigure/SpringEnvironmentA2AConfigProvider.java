 ...
 *
 * <p>
 * Spring property sources win first. If a key is absent there, this provider falls back
 * to the default values bundled with the SDK via {@link DefaultValuesConfigProvider}.
 * That keeps the runtime behavior identical to the core server while still letting Spring
 * Boot override values from {@code application.yml}, environment variables, command-line
 * args, or any other Spring property source.
 */
final class SpringEnvironmentA2AConfigProvider implements A2AConfigProvider {
	...
	SpringEnvironmentA2AConfigProvider(Environment environment,
