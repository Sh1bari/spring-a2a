package io.github.sh1bari.springa2a.springboot.server.autoconfigure;
 ...
 *
 * <p>
 * This configuration owns the core server runtime beans only: config providers, task
 * store, event bus, queue manager, push-notification infrastructure, internal executors,
 * and the default {@link RequestHandler} wiring when an {@link AgentExecutor} is
 * available.
 *
 * <p>
 * The class deliberately does not depend on Servlet or Spring MVC APIs so the runtime
 * layer can activate in non-web Spring Boot applications as well.
 */
@AutoConfiguration
...
public class A2ARuntimeAutoConfiguration {
	...
	public A2AConfigProvider a2aConfigProvider(Environment environment,
	...
	public ExecutorService a2aInternalExecutor(A2ASpringBootProperties properties) {
		...
		ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(executor.getCorePoolSize(),
				executor.getMaxPoolSize(), executor.getKeepAliveSeconds(), TimeUnit.SECONDS,
				new ArrayBlockingQueue<>(executor.getQueueCapacity()), new A2AInternalThreadFactory());
	...
	public ExecutorService a2aEventConsumerExecutor() {
		return new ThreadPoolExecutor(0, Integer.MAX_VALUE, 10, TimeUnit.SECONDS,
				new java.util.concurrent.SynchronousQueue<>(), new A2AEventConsumerThreadFactory());
	}

	@Bean
	@ConditionalOnMissingBean(RequestHandler.class)
	@ConditionalOnSingleCandidate(AgentExecutor.class)
	public RequestHandler requestHandler(ObjectProvider<AgentExecutor> agentExecutorProvider, TaskStore taskStore,
			QueueManager queueManager, PushNotificationConfigStore pushNotificationConfigStore,
			MainEventBusProcessor mainEventBusProcessor, @Qualifier("a2aInternalExecutor") Executor internalExecutor,
			@Qualifier("a2aEventConsumerExecutor") Executor eventConsumerExecutor) {
		AgentExecutor agentExecutor = agentExecutorProvider.getIfUnique();
		if (agentExecutor == null) {
			throw new IllegalStateException("AgentExecutor must be available when RequestHandler is created.");
		}
