...
public class SpringBootRestClientDemoService {
	...
	public SpringBootRestClientScenarioResponse runBlockingDemo(SpringBootRestClientDemoRequest request) {
		...
		try {
			...
			return SpringBootRestClientScenarioResponse.failure("blocking", properties.serverUrl(), messageText,
	...
	public SpringBootRestClientScenarioResponse runStreamingDemo(SpringBootRestClientDemoRequest request) {
		...
		try {
			...
			return SpringBootRestClientScenarioResponse.failure("streaming", properties.serverUrl(), messageText,
					List.of(), e.getMessage());
		}
	}
	...
	private Client createClient(AgentCard agentCard, boolean streaming) throws Exception {
		ClientConfig clientConfig = new ClientConfig.Builder().setStreaming(streaming).build();
	...
	private String describeEvent(String scenario, ClientEvent event, AgentCard agentCard) {
		if (event instanceof MessageEvent messageEvent) {
			return "[" + scenario + "] MessageEvent from " + agentCard.name() + ": "
					+ extractText(messageEvent.getMessage());
		}
		if (event instanceof TaskEvent taskEvent) {
			return "[" + scenario + "] TaskEvent: taskId=" + taskEvent.getTask().id() + ", state="
					+ taskEvent.getTask().status().state();
		}
		if (event instanceof TaskUpdateEvent taskUpdateEvent) {
			return "[" + scenario + "] TaskUpdateEvent: taskId=" + taskUpdateEvent.getTask().id() + ", state="
